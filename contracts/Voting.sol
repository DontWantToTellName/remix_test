...
contract ProposalContract {
    ...
    address[] private voted_addresses; // Array to track addresses that have voted
    ...
    constructor() {
        owner = msg.sender;
        voted_addresses.push(msg.sender); // Add owner to voted addresses
    }
    ...
    // Modifier to check if the voter has already voted
    ...
    function vote(uint8 choice) external {
        ...
        if ((proposal.total_vote_to_end - total_vote == 1) && (choice == 1 || choice == 2 || choice == 0)) {
            ...
            voted_addresses = [owner]; // Reset the voted addresses to only include the owner
        }
    }

    // Function to calculate the current state of the proposal with custom logic
    ...
    // Function to check if a given address has voted
    ...
    // Function to retrieve a specific proposal by ID
